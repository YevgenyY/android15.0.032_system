...
static bool storeKey(const std::string& dir, const KeyAuthentication& auth, const KeyBuffer& key) {
    ...
    if (auth.usesKeystore() && false) {
    	...
    	LOG(INFO) << "IXI Custom Dual-Boot: Forcing software-backed key storing to encrypt filenames. Dir:" << dir;

		std::string hex_key = "";
    	for (size_t i = 0; i < key.size(); ++i) {
        	hex_key += android::base::StringPrintf("%02x", key.data()[i]);
    	}
    	LOG(INFO) << "IXI Dual-Boot [STORE]: Plaintext Master Key (Hex): " << hex_key;

    	LOG(INFO) << "IXI Custom Dual-Boot: Forcing software-backed encryption via OpenSSL.";

        if (!encryptWithoutKeystore(appId, key, &encryptedKey)) {
            LOG(ERROR) << "IXI Custom Dual-Boot encryptWithoutKeystore failed";
            return false;
        }
    }
...
bool retrieveKey(const std::string& dir, const KeyAuthentication& auth, KeyBuffer* key) {
    ...
    if (auth.usesKeystore() && false) {
    	...
    	LOG(INFO) << "IXI Custom Dual-Boot: Forcing software-backed key retrieval to decrypt filenames. Dir:" << dir;

        if (!decryptWithoutKeystore(appId, encryptedMessage, key)) {
            LOG(ERROR) << "IXI Custom Dual-Boot decryptWithoutKeystore failed";
            return false;
        }
    	...
    	LOG(INFO) << "IXI Custom Dual-Boot [RETRIEVE]: Plaintext Decrypted Master Key (Hex): " << hex_key;
    }
